      ...
      if (200 == result.status && result.data.length){
        ...
        for(var status in result.data) {
          ...
          if (!options || options && options.silent != true) {
            console.log(result.data[status].id_str);
          }
        }
      }
    ...
    var callback = function(result) {
      if (200 == result.status && result.data.length){
        ...
        for(var direct_messages in result.data) {
          ...
          if (!options || options && options.silent != true) {
            console.log(result.data[direct_messages].id_str);
          }
        }
      }
    ...
    if (my.current_requests.verify_credentials) {
      ...
      if (200 == result.status && result.data){
        ...
        if (Location.enabled && !result.data.geo_enabled) {
          console.log('Location is currently disabled in your <a target="_blank" href="https://twitter.com/settings/account">Twitter settings</a>. If you would like to use location features be sure to enable it.')
        }
      }
  ...
  my.update = function(status, options) {
    ...
    var callback = function(result) {
      if (200 == result.status){
        ...
        console.log('Grrr... Something went wrong. #fail!', result.data.error)
      }
    }
